...
Для этого достаточно открыть ссылку из списка ниже и выбрать нужную версию среды выполнения.

Чтобы выбрать версию среды выполнения, во вкладке `Среда выполнения` (`Runtime`) выберите `Сменить среду выполнения` (`Change runtime type`), в открывшемся окне поменяйте версию на **`2025.07`**.
Теперь вы моежете последовательно выполнить ячейки блокнота-примера.
